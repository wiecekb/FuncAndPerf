...
test.describe('Step Data Registry - Unit Tests', (): void => {
  test('5.1.1 Registry stores and retrieves data', (): void => {
    ...
    const record: StepDataRecord | undefined = stepDataRegistry.get('step1');
  ...
  test('5.1.5 Registry overwrites existing handler name', (): void => {
    ...
    const record: StepDataRecord | undefined = stepDataRegistry.get('dup');
...
test.describe('Resolve References - Unit Tests', (): void => {
  ...
  test('5.2.10 resolveReference throws for unknown handler', (): void => {
    expect(() => resolveReference('unknownStep.response.$.system.resultCode')).toThrow(
      /Step data handler "unknownStep" not found/
    );
  });

  test('5.2.11 resolveReference throws for unknown source', (): void => {
    expect(() => resolveReference('step1.auth.$.token')).toThrow(/Source "auth" not found/);
  });

  test('5.2.12 resolveReference throws for non-existent jsonpath', (): void => {
    expect(() => resolveReference('step1.response.$.nonExistent.path')).toThrow(/resolved to undefined\/null/);
  });

...
async function getTestPointsForCase(
  ...
  if (result.status && result.status >= 200 && result.status < 300) {
    ...
    console.warn(
      `  Failed to get test points for case ${testCaseId}: ${result.status} - ${JSON.stringify(result.data)}`
    );
    return [];
  }
}
...
async function completeTestRun(orgUrl: string, project: string, token: string, runId: number): Promise<unknown> {

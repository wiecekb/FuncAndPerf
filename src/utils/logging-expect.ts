...
class LoggingExpect {
    ...
    private async logAssertion(
        assertionType: string,
        details: { expected?: unknown; negated?: boolean }
    ): Promise<void> {
        try {
            ...
            await attachment(
                `Assertion Details: ${assertionType}`,
                JSON.stringify(logData, null, 2),
                'application/json'
            );
        } catch (error) {
            throw new Error(`Failed to log assertion to Allure: ${error instanceof Error ? error.message : String(error)}`, {cause: error});
        }
    }

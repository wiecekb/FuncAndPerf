...
const USERS_FILE_PATH: string = process.env.MOCK_USERS_FILE || path.resolve(process.cwd(), 'users.txt');
...
function loadUsers(filePath: string): MockUser[] {
    ...
    return content
        ...
        .filter((user: { username: string; password: string }):boolean => Boolean(user.username) && Boolean(user.password));
}
...
app.post('/oauth/token', (req: Request, res: Response) => {
    ...
    const user: MockUser | undefined = users.find((candidate: MockUser):boolean => candidate.username === username && candidate.password === password);
    if (!user) {
...
app.post('/api/calc/add', calcHandler('add', (a: number, b: number): number => a + b, false));
app.post('/api/calc/multiply', calcHandler('multiply', (a: number, b: number): number => a * b, false));
app.post('/authorized/api/calc/add', calcHandler('add', (a: number, b: number): number => a + b, true));
app.post('/authorized/api/calc/multiply', calcHandler('multiply', (a: number, b: number): number => a * b, true));
